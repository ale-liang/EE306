...
APPLY_MOVE   
    ...
    JSR SAFE_MOVE               ;after SAFE_MOVE finishes, R1 will have correct column number and R2 will have correct row number to move 
    LD R7, saveR0ApplyMove
    ADD R1, R1, #0
    BRzp moveIsSafe
...
moveIsSafe
    ...
    BRz moveRight
moveUp
    STR R4, R6, #0          ;replace current Simba with a blank space char
    ADD R6, R6, #-16
    ADD R6, R6, #-2
    STR R4, R6, #0          ;replace line with a blank space char
    ADD R6, R6, #-16
    ADD R6, R6, #-2
    STR R5, R6, #0          ;place Simba char into this spot
    ST R1, CURRENT_ROW
    BR finishApplyMove
moveLeft
    STR R4, R6, #0
